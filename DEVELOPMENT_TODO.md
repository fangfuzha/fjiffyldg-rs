...
- **修复方案**：
  ...
  3. 添加分块索引的二分查找逻辑
- **测试**：已添加百万行后续偏移保存、chunk 填充、chunk 溢出记录、按位置查询范围裁剪单元测试
- **状态**：🔄 部分完成（已修复直接索引上限后的扩展索引承接，补上 chunk 填充与 overstep 记录，并让 `get_line_by_pos()` 使用 chunk 裁剪二分范围；`get_line_pos()` 仍依赖直接/扩展精确偏移）
- **复杂度**：高
...
| -------- | ------ | ------------------------- | ------------------ |
| 第一阶段 | 3      | 3                         | 100%               |
| 第二阶段 | 4      | 2（另 1 个部分完成）      | 50% + 部分完成     |
| 第三阶段 | 3      | 3                         | 100%               |
| 第四阶段 | 5      | 5                         | 100%               |
| **总计** | **15** | **13（另 1 个部分完成）** | **86% + 部分完成** |
...
**最后更新**：2026-05-19（已同步第一阶段、百万行索引修复、chunk 填充与按位置查询范围裁剪、overstep 查询范围裁剪、UTF-8 offset、read_line_cut、UTF-16 offset 重扫、UTF-16BE/UTF-32 行扫描补测、加载状态 API、Condvar 扫描等待与 C FFI 核心绑定）
**维护者**：开发团队
