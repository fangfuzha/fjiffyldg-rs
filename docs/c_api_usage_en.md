...
| `ToSaveFile`           | `int ToSaveFile(const char *fileName, const char *buffer, long long len)`    | `fileName`: destination path; `buffer`: bytes to write; `len`: byte count           | `0` on success; non-zero error code otherwise | Writes `buffer[0..len)` to the file; a null buffer is allowed when `len == 0`; `len < 0` returns an error     |
| `ToAppendFile`         | `int ToAppendFile(const char *fileName, const char *buffer, long long len)`  | `fileName`: destination path; `buffer`: bytes to append; `len`: byte count          | `0` on success; non-zero error code otherwise | Creates the file if missing; a null buffer is allowed when `len == 0`; the caller keeps ownership of `buffer` |
