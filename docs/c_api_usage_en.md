...
`ReadFileDataLLineCut` follows the C++ reference semantics: it batches short lines and truncates long lines at the 4 KB cutoff. `ReadFileDataEndOfLine` reads from a byte position to the current line end; when `pos` is already at that line end, it returns an empty buffer and writes `len` back as `0`.
