...
| `GetFileLinePos`    | `long long GetFileLinePos(fjiffyldg_ptr fm, long long index)`    | `fm`: handle; `index`: zero-based line number | Line start byte offset; `-1` on failure or out of range                                                                          | Offsets are raw file bytes, not character positions; line `0` of an empty file starts at `0`                                                                                                                           |
