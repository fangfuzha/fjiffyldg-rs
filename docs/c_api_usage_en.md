...
When `pos` is at or beyond the end of the file, the behavior matches the C++ reference implementation: the position is clamped to EOF, an empty buffer is returned, and `len` is written back as `0` instead of treating the call as a hard read failure.
