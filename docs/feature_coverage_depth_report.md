...
| Queries while scanning is in progress            | `GetFileLinePos` may expose built prefixes; `GetFileLineIndex` remains unavailable   | Done        | `get_file_line_pos_exposes_scanned_prefix_while_scanning`, `get_file_line_index_stays_unavailable_while_scanning`           |
