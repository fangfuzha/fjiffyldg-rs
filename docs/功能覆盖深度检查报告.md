...
**检查日期**：2026-05-19  
**总体评估**：覆盖约 86% 核心功能，基础架构完整，首批关键正确性缺口已修复，剩余重点集中在 C FFI、真正的分块查询与大文件 I/O 优化
...
| 5   | 后台扫描克隆整文件             | 大文件扫描多一份内存   | 优先用 `Arc<[u8]>` 或 mmap 引用共享 |
...
Rust 版本已覆盖常见文件加载、扫描、行定位、编码检测与读取场景。2026-05-19 的修复消除了几处会导致行为不一致的正确性问题：CRLF、UTF-16LE/BE 与 UTF-32LE/BE 原始字节偏移、UTF-32 BOM、重扫 API、百万行后续偏移、`ReadFileDataLLineCut` 对应读取语义、UTF-8 消费字节数、加载状态诊断和扫描完成通知等待。

后续开发应优先进入结构性能力补齐：C FFI、真正的 chunk 索引、windowed mmap，以及可取消的后台扫描。
