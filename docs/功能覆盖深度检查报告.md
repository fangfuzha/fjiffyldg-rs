...
**检查日期**：2026-05-19  
**总体评估**：覆盖约 94% 核心功能，基础架构完整，首批关键正确性缺口已修复，剩余重点集中在真正的分块查询、windowed mmap 与极端溢出处理
...
| 溢出处理 | `overstep` 字段       | `overstep_pos` 记录首个溢出位置 | ⚠️   | 已记录，查询侧仍待完整接入             |
...
- **分块查询仍未完全等同 C++**：`chunks` 已在扫描时填充，`get_line_by_pos()` 已使用 chunk 裁剪二分范围，`overstep_pos` 已记录首个溢出位置，但尚未复刻 C++ 从 chunk 起点回扫目标行的完整路径
- **性能结构差异**：当前直接索引与扩展索引可保持百万行后的正确性，按位置查询已能利用 chunk 缩小搜索范围，但行号到位置查询仍直接依赖完整偏移数组
- **已修复正确性缺口**：`CHUNK_COUNT_MAX`、CRLF 行长度、UTF-16LE/BE 与 UTF-32LE/BE 换行宽度、百万行后续偏移保存、chunk 填充与按位置查询范围裁剪均已补单元测试
...
Rust 版本已覆盖常见文件加载、扫描、行定位、编码检测与读取场景。2026-05-19 的修复消除了几处会导致行为不一致或资源浪费的问题：CRLF、UTF-16LE/BE 与 UTF-32LE/BE 原始字节偏移、UTF-32 BOM、重扫 API、百万行后续偏移、chunk 填充与按位置查询范围裁剪、`ReadFileDataLLineCut` 对应读取语义、UTF-8 消费字节数、加载状态诊断、扫描完成通知等待、扫描中止、后台扫描前的整块数据复制，以及大文件 clone/save/append/concatenate 的慢路径。

后续开发应优先进入结构性能力补齐：完整 C++ 风格 chunk 回扫路径、windowed mmap 与 `overstep_pos` 溢出处理。C FFI 已具备核心调用闭环，后续可继续补独立 C 集成测试和发布用头文件。
