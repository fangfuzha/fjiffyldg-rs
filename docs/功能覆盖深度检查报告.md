...
| 第三层   | 分块索引 (128KB/块)   | `chunks: Vec<ChunkIndex>`    | ⚠️   | 已填充状态，查询路径仍未完整接入 |
| 分块限制 | 8,388,608 (8M)        | 8,388,608                    | ✅   | 已修正为 C++ 同量级   |
| 溢出处理 | `overstep` 字段       | `overstep_pos` 记录首个溢出位置 | ⚠️   | 已记录，查询侧仍待接入 |
...
- **分块查询未完整接入**：`chunks` 已在扫描时填充，`overstep_pos` 已记录首个溢出位置，但查询函数尚未完整走第三层路径
...
Rust 版本已覆盖常见文件加载、扫描、行定位、编码检测与读取场景。2026-05-19 的修复消除了几处会导致行为不一致或资源浪费的问题：CRLF、UTF-16LE/BE 与 UTF-32LE/BE 原始字节偏移、UTF-32 BOM、重扫 API、百万行后续偏移、`ReadFileDataLLineCut` 对应读取语义、UTF-8 消费字节数、加载状态诊断、扫描完成通知等待、扫描中止、后台扫描前的整块数据复制，以及大文件 clone/save/append/concatenate 的慢路径。

后续开发应优先进入结构性能力补齐：真正的 chunk 索引、windowed mmap 与 `overstep_pos` 溢出处理。C FFI 已具备核心调用闭环，后续可继续补独立 C 集成测试和发布用头文件。
