...
| C++ 头文件 RAII 包装 smoke 编译               | ✅   | `cbindgen.toml`、`include/fjiffyldg.h`、`tests/cpp_smoke.cpp`、`scripts/check_c_abi.ps1`                                                                                                 |
...
当前已补 Criterion 大文件基准入口，覆盖 mmap 加载扫描、随机行查询与随机读取；也已提供由 `cbindgen` 生成的 C/C++ ABI 头文件、smoke 编译脚本、C API 双语使用文档和 `GetFileMappedHuge` 真实 mmap 指针语义。后续开发应优先转入发布配套：在真实超大文件环境扩展基准，并补更完整的独立 C 链接/运行集成测试。
