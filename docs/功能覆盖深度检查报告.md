...
**检查日期**：2026-05-19  
**总体评估**：覆盖约 95% 核心功能，基础架构完整，首批关键正确性缺口已修复，剩余重点集中在完整 C++ 风格分块回扫路径与 windowed mmap
...
Rust 版本已覆盖常见文件加载、扫描、行定位、编码检测与读取场景。2026-05-19 的修复消除了几处会导致行为不一致或资源浪费的问题：CRLF、UTF-16LE/BE 与 UTF-32LE/BE 原始字节偏移、UTF-32 BOM、重扫 API、百万行后续偏移、chunk 填充与按位置查询范围裁剪、overstep 溢出段范围裁剪、`ReadFileDataLLineCut` 对应读取语义、UTF-8 消费字节数、加载状态诊断、扫描完成通知等待、扫描中止、后台扫描前的整块数据复制，以及大文件 clone/save/append/concatenate 的慢路径。

后续开发应优先进入结构性能力补齐：完整 C++ 风格 chunk 回扫路径与 windowed mmap。C FFI 已具备核心调用闭环，后续可继续补独立 C 集成测试和发布用头文件。
