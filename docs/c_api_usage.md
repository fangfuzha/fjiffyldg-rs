...
| `RestartScanFile`          | `void RestartScanFile(fjiffyldg_ptr fm, const char *name, long long offset, int utf)` | `fm`：句柄；`name`：可用于重新加载/扫描的路径；`offset`：扫描起始字节；`utf`：UTF 模式编号 | 无                                                | 会停止当前扫描，使用传入 `name` 重新加载并从指定偏移重建索引；无效句柄或路径会被忽略 |
