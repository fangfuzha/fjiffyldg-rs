...
| 接口                | 签名                                                             | 参数                            | 返回值                                                                   | 生命周期与注意事项                                                                                        |
| ------------------- | ---------------------------------------------------------------- | ------------------------------- | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `GetFileLineCount`  | `long long GetFileLineCount(fjiffyldg_ptr fm)`                   | `fm`：句柄                      | `>0` 表示扫描完成后的总行数；`0` 表示尚未开始扫描；`<0` 表示扫描中或失败 | `LoadFileOnly` 后未构建行索引时返回 `0`；后台扫描进行中通常返回负数；空文件完成扫描后按 C++ 语义视为 1 行 |
| `GetFileLinePos`    | `long long GetFileLinePos(fjiffyldg_ptr fm, long long index)`    | `fm`：句柄；`index`：0 起始行号 | 行起始字节偏移；失败或越界返回 `-1`                                      | 偏移基于原始文件字节，不是字符数；空文件的第 0 行起始位置为 `0`                                           |
