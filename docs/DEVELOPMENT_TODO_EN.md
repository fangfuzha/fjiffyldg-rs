...
**Progress:** Added the first C ABI boundary regressions and fixed eighteen Rust mismatches: `GetFileLineIndex` no longer returns `0` when no line offsets have been built, it no longer falls back to the last line when the queried byte position is past the end of the file, `ReadFileData` now clamps negative positions to the file start and returns an empty buffer at EOF / past EOF instead of incorrectly reporting failure, `ReadFileDataEndOfLine` no longer misreports failure or reads into the next segment when the caller is already at the current line boundary or the end of the file, `ReadFileDataLLineCut` no longer overwrites the caller's existing bounds when lookup fails and now returns an empty buffer instead of a failure for valid empty ranges, `GetFileMappedHuge` no longer keeps the previous mapping alive after a later mapping attempt fails, empty files now expose the same observable single-empty-line state as the C++ reference, empty files on the `LoadFileOnly` path no longer incorrectly enter a scanned state, `GetFileLinePos` now exposes already-built prefix positions while background scanning is still in progress, `GetFileLineLength` no longer exposes line lengths before scanning has completed, file helpers no longer leak Rust-internal error codes where the C++ public contract requires `-1` for severe failures, `CheckExtractTextUtf8` now matches the C++ extract-check handling for UTF-8 characters truncated at slice boundaries, `RestartScanFile` no longer ignores the caller-provided `name` after a file has already been loaded, and missing-file loads now return the C++ reference error value `-1` instead of the inaccessible-file code.
