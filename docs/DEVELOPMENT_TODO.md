...
- **进展**：已补首批 C ABI 边界回归测试，并修正十六处 Rust 偏差：未建立任何行偏移时 `GetFileLineIndex` 错误返回 `0`，查询位置超过文件末尾时误返回最后一行，`ReadFileData` 在负偏移时未钳到文件起点、在 EOF/越过 EOF 时错误返回失败而非空缓冲区，`ReadFileDataEndOfLine` 在行尾边界与文件末尾位置误返回失败或误读到下一段数据，`ReadFileDataLLineCut` 失败时错误覆盖调用方已有边界值、在空文件空读场景误返回失败，`GetFileMappedHuge` 在失败调用后错误保留旧映射，空文件未按 C++ 语义暴露“1 行、偏移 0、长度 0”的可观察状态，空文件在 `LoadFileOnly` 路径下错误提前进入“已扫描”状态，扫描进行中时 `GetFileLinePos` 未暴露已扫描前缀，`GetFileLineLength` 在扫描完成前错误提前返回行长度，文件 helper 在严重失败场景下错误泄露 Rust 内部错误码而非 C++ 公开约定的 `-1`，以及 `CheckExtractTextUtf8` 对片段首尾截断 UTF-8 字符处理与 C++ 抽取式检查不一致的问题。
- **状态**：⏳ 持续执行
