...
- **进展**：已补首批 C ABI 边界回归测试，并修正九处 Rust 偏差：未建立任何行偏移时 `GetFileLineIndex` 错误返回 `0`，查询位置超过文件末尾时误返回最后一行，`ReadFileData` 在负偏移时未钳到文件起点、在 EOF/越过 EOF 时错误返回失败而非空缓冲区，`ReadFileDataEndOfLine` 在行尾边界与文件末尾位置误返回失败或误读到下一段数据，`ReadFileDataLLineCut` 失败时错误覆盖调用方已有边界值，`GetFileMappedHuge` 在失败调用后错误保留旧映射，以及空文件未按 C++ 语义暴露“1 行、偏移 0、长度 0”的可观察状态。
- **状态**：⏳ 持续执行
